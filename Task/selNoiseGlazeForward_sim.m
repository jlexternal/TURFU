...
% Selection Noise
% 
%   Name:   selNoiseGlazeForward_sim.m
% Author:   Jun Seok Lee
%   Date:   April 2018
%   Type:   Script
...
load('selNoiseParamsLog','selNoiseParamsLog'); % this could be changed based on which parameter set is being used
...
for isubj = subjects
    ...
    simBehaviorForward(isubj).pastParams    = selNoiseParamsLog(isubj).past;
    ...
    for block = blocks
        ...
        simBehaviorForward(isubj).rslt(block).resp = selNoiseGlazeForward_simulator(haz, sigm, eta, seqllrs, nsamples);
        ...
        corTrls = simBehaviorForward(isubj).rslt(block).resp(2:73)-expe.rslt(block).resp(2:73); % matching trials
