function objFn = selNoiseGlaze_PF_recov(params, getmodelLL, futurecond)
% Sequential Monte Carlo Particle Filter on Modified Glaze (2015)
%
%   Name:   selNoiseGlaze_PF_recov
%   Type:   Function
% Author:   Jun Seok Lee
%   Date:   May 2019
%   Note:   This code is made to run on data generated from the TURFU experiment.
...
eta     = params(3);            % Modified for BADS
...
x1 = evalin('base', 'x1');
if txt ~= 1
    x2 = [hazRate sigma eta];
    v = x2-x1;
    quiver3(x1(1),x1(2),x1(3),v(1),v(2),v(3),0,'Color','r');
end
drawnow;
assignin('base', 'x1', [hazRate sigma eta]);
scatter3(hazRate, sigma, eta, '.');
hold on;
text(hazRate, sigma, eta, num2str(txt));
set(gca,'XLim',[0 1],'YLim',[0 2],'ZLim',[0 2]);
drawnow;
...
%       infNoise : Gaussian white noise whose stDev = sigma*sqrt(m_j)
%           := random draw from Gaussian(0, sigma*sqrt(m_j))
%   selNoise: Gaussian white noise
%           := random draw grom Gaussian(0, eta)

psi         = @(lnm1, haz) lnm1 + log((1-haz)./haz + exp(-lnm1)) - log((1-haz)./haz + exp(lnm1));
infNoise    = @(sigm,nsamples) random('Normal', 0, sigm*sqrt(nsamples));
selNoise    = @(eta) random('Normal',0,eta);
...
for i = blocks
    ...
    for j = 1:n_trials
      ...
      % Simulation of L's for however many particles is set
        ...
        % Inference Model (Glaze + infNoise on llr) with Selection Noise
        for k = 1:n_part_temp
            if j == 1   % first instance of L has no psi
                L       = LLR   + infNoise(sigma, nSmpls); % belief based on Glaze + Inference Noise
            else 
                L       = LLR   + infNoise(sigma, nSmpls) + psi(L_acc_btemp{1,j-1}(k), hazRate); % belief based on Glaze + Inference Noise
            end
            if futurecond
                L = psi(L,hazRate);
            end
            L_sel   = L + selNoise(eta);        % belief based on above + Selection Noise (this does not pass through as the posterior)
            L_ttemp = horzcat(L_ttemp, L);
            
            % Filter particles that correspond to the subject choice 
            if (posDirBool == true & L_sel > 0) | (posDirBool == false & L_sel < 0)
